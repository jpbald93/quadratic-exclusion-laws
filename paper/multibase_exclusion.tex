...
\begin{abstract}
Say that an odd prime $p$ is \emph{Artin base $a$} if $a$ is a primitive root
...
already record. All results above
...
abundant outside the exclusion classes, but abundance in a finite range is not
...
quadratic exclusion classes, with the converse direction included.
...
effect to that reference scale, not as a test of any hypothesis. The precision
...
refutation of the exclusion-density hypothesis, and the conductor
...
Two features stand out. First, $\delta(a) < 0$ for every base: consecutive
primes \emph{repel} in Artin status, uniformly in the base. This is consistent
...
and hence through $\omega(p-1)$, whose consecutive values are themselves
anticorrelated --- into Artin status.
...
suffices to model the covariance. Assuming that $X$ and $Y$ are conditionally
independent given the residue pair $(p_n, p_{n+1}) \bmod f$ --- an
approximation, since the factorizations of $p_n - 1$ and $p_{n+1} - 1$ share
structure not captured by residues modulo $f$ --- one obtains
\[
...
of the Artin density in the class $r$ from its mean; a residual term from the
failure of conditional independence has been dropped. The first factor is
...
We have not done this here: the LOS transition term for \emph{consecutive}
(rather than merely distinct) primes in fixed residue classes involves a
secondary term with no closed form, and the interaction with the
$\omega(p-1)$ channel (through which the bias reaches Artin status) is not
...
narrower band, so $f^{-1/2}$ is the better one-parameter description, but
neither product is constant and neither varies monotonically with $f$. The
...
admits no exclusion law. The data refuted the assertion --- twenty million
consecutive pairs at gaps $g \equiv 2, 3 \pmod 5$ with a zero doubly-Artin
count is not a subtle signal --- and the inadmissibility mechanism was found
by asking what the classification had missed. That same earlier version
...
The author used a large-language-model AI assistant in the preparation of this
work: for drafting and revising exposition, for writing and debugging the
sieve and analysis programs, for exploratory discussion of the
classification, and for the Lean formalisation described in
Section~\ref{sec:formal}. The present revision was also prepared with such
assistance. Several specific interventions should be recorded in the interest
of transparency. An initial machine-generated form of
Table~\ref{tab:components} incorrectly classified the conductor-$8$ component
as preserving at $g \equiv 2 \pmod 8$; the error was detected by exhaustive
computational comparison and corrected before any proof was written. The
hypothesis refuted in Observation~\ref{obs:refutation} was originally
proposed, with a plausible heuristic, in the course of that assisted
exploration; it was tested and rejected on the evidence. Conversely, three
errors that survived into a previous version of this paper --- the missing
hypothesis in Theorem~\ref{thm:inadmissible}, the invalid composite case of
Corollary~\ref{cor:complete}, and a false assertion about twin primes in the
discussion --- were identified during a subsequent assisted audit and are
corrected here; the counting identity that repairs the second is machine-checked, as are the
$d = 5$ and $d = 13$ cases of the first; the general-$d$ count and the
composite dichotomy remain pen-and-paper proofs. The author is solely
responsible for the content of this paper, including any remaining errors.
